# ─────────────────────────────────────────────────────────────────
# SenfoniScan: Otomatik bağımlılık kurulum kodu
# Arch/Debian gibi korumalı sistem Python'larında otomatik .venv
# oluşturur ve kendini venv Python'u ile yeniden başlatır.
# ─────────────────────────────────────────────────────────────────
import sys
import subprocess
import importlib
...
_VENV_MARKER  = os.path.join(_VENV_DIR, "senfoniscan_ready")   # boş dosya, kurulum tamamlanınca oluşur
...
def _c(code, msg):
    """Renkli terminal çıktısı (rich yüklü olmadan)."""
    print(f"\033[{code}m{msg}\033[0m")

def _ensure_venv_and_reexec():
    """
    Eğer .venv yoksa veya kurulum henüz tamamlanmamışsa:
    1) venv oluştur
    2) paketleri kur
    3) playwright chromium kur
    4) marker dosyası yaz
    5) bu süreci venv Python'u ile yeniden başlat (exec)
    """
    ...
    if not os.path.isfile(_VENV_MARKER):
        ...
        subprocess.run(
            [_VENV_PYTHON, "-m", "playwright", "install", "chromium"],
            check=False
        )

        # Marker yaz — bir daha tekrar kurulmasın
        open(_VENV_MARKER, "w").close()
        _c(92, "[✔] Tüm bağımlılıklar başarıyla kuruldu!\n")

    # Bu süreci .venv Python'u ile yeniden başlat
    os.execv(_VENV_PYTHON, [_VENV_PYTHON] + sys.argv)


# Eğer şu an venv içinde DEĞİLSEK → kurulumu yap ve yeniden başlat
_in_venv = (
...
if not _in_venv:
    # Sistem Python ile çalışıyoruz — venv'e geç
    _ensure_venv_and_reexec()
else:
    # venv içindeyiz: sadece eksik paketleri kontrol et (normalde yok)
    _missing = []
    ...
    if _missing:
        _c(93, f"[*] Eksik paket tespit edildi: {', '.join(_missing)} — kuruluyor...")
